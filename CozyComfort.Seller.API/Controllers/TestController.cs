...
namespace CozyComfort.Seller.API.Controllers
{
    ...
    public class TestController : ControllerBase
    {
        ...
        /// <summary>
        /// Reset all default passwords (DEVELOPMENT ONLY)
        /// </summary>
        [HttpPost("reset-all-default-passwords")]
        [AllowAnonymous]
        public async Task<IActionResult> ResetAllDefaultPasswords()
        {
            try
            {
                ...
                var admin = await _context.Users.FirstOrDefaultAsync(u => u.Email == "admin@seller.com");
                if (admin != null)
                {
                    admin.PasswordHash = BCrypt.Net.BCrypt.HashPassword("Admin123!");
                    admin.UpdatedAt = DateTime.UtcNow;
                    results.Add(new { email = admin.Email, password = "Admin123!", status = "Reset" });
                }

                // Reset seller password
                var seller = await _context.Users.FirstOrDefaultAsync(u => u.Email == "seller@cozycomfort.com");
                if (seller != null)
                {
                    seller.PasswordHash = BCrypt.Net.BCrypt.HashPassword("Seller123!");
                    seller.UpdatedAt = DateTime.UtcNow;
                    results.Add(new { email = seller.Email, password = "Seller123!", status = "Reset" });
                }
            ...
            {
                _logger.LogError(ex, "Error resetting default passwords");
                return StatusCode(500, new { error = ex.Message });
            }
        }
    ...
    public class CreateUserRequest
    {
        ...
        public CozyComfort.Shared.Enums.UserRole Role { get; set; }
